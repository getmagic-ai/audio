...
function NewsletterForm({ currentUser }) {
  ...
  const [prefObj, setPrefObj] = useState({ blog: true, email: "", promotions: true, updates: true }); //checkboxes state
  ...
  // a function to validate the email address entered below
  ...
  async function subscribe(event) {
    ...
    //to send the subcription confirmation email
    setRecipient(email)
    setIsSending(true);
    fetch("api/sendgrid", {
      method: "post",
    ...
    //------------fetch data from userPrefDb------------
    // const getEmailSubs = async (userPrefDb) => {
    //   const emailSubsCol = collection(userPrefDb, 'users');
    ...
    // }
    // getEmailSubs(userPrefDb);


    //---------------add data to userPrefDb-------------------
    const addEmailSub = async (userPrefDb) => {
      const emailSubsCol = collection(userPrefDb, 'Newsletter_subscribers');
      await setDoc(doc(emailSubsCol, email), prefObj);
    }

    addEmailSub(userPrefDb);
